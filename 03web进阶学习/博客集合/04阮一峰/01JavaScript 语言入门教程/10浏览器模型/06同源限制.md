...
举例来说，`http://www.example.com/dir/page.html`这个网址，协议是`http://`，域名`是www.example.com`，端口是`80`（默认端口可以省略），它的同源情况如下。
...
如果两个窗口一级域名相同，只是二级域名不同，那么设置上一节介绍的d`ocument.domain`属性，就可以规避同源政策，拿到 DOM。
...
- David Baron, [setTimeout with a shorter delay](https://dbaron.org/log/20100309-faster-timeouts): 利用 window.postMessage 可以实现0毫秒触发回调函数
