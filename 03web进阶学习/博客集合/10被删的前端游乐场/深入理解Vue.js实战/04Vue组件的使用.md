...
## 2. 组件使用
组件是可复用的 Vue 实例，所以它们与`new Vue()`接收相同的选项，例如`data`、`computed`、`watch`、methods以及生命周期钩子等（生命周期一致，可以参考第3章中 Vue 实例的生命周期）。除了`el`这种是根实例特有的选项。
...
`required`	|定义该 `prop` 是否是必填项，默认为false	|在非生产环境中，如果这个值为`true`且该`prop`没有被传入的，则一个控制台警告将会被抛出
