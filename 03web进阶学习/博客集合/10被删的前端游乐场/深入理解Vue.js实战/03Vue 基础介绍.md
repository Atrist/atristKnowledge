...
```
## 3. 计算属性和侦听器
`computed`和`watch`大概是 Vue 里除了`data`、`methods`、`props`之外，用的最多的选项了。前面我们也讲了很多次，Vue 中数据变更之所以能更新到页面里，因为对`data`执行 `getter/setter` 转化，然后进行侦听。而`computed和watch`属性都是基于此，我们看看一个`Watcher`是怎样的：
```js
