...
>注意：<br/>
>在组件中有些特殊的规则，规定什么地方能使用 Hoo>，什么地方不能使用。我们将在 [Hook 规则](https://react.docschina.org/docs/hooks-rules.html)中学习它们。
# 声明 State 变量
...
**useState 方法的返回值是什么？** 返回值为：当前 state 以及更新 state 的函数。这就是我们写 `const [count, setCount] = useState()` 的原因。这与 class 里面 `this.state.count` 和 `this.setState` 类似，唯一区别就是你需要成对的获取它们。如果你不熟悉我们使用的语法，我们会在[本章节的底部](https://react.docschina.org/docs/hooks-state.html#tip-what-do-square-brackets-mean)介绍它。
...
>注意 <br/>
>你可能想知道：为什么叫 `useState` 而不叫 `createState`? <br/>
>“Create” 可能不是很准确，因为 state 只在组件首次渲染的时候被创建。在下一次重新渲染时，`useState` 返回给我们当前的 state。否则它就不是 “state”了！这也是 Hook 的名字总是以 `use` 开头的一个原因。我们将在后面的 [Hook 规则中](https://react.docschina.org/docs/hooks-rules.html)了解原因。
...
>**注意**<br/>
>你可能会好奇 React 怎么知道 `useState` 对应的是哪个组件，因为我们并没有传递 `this` 给 React。我们将在 FAQ 部分回答[这个问题](https://react.docschina.org/docs/hooks-faq.html#how-does-react-associate-hook-calls-with-components)以及许多其他问题。
...
我们[在 FAQ 中](https://react.docschina.org/docs/hooks-faq.html#should-i-use-one-or-many-state-variables)提供了更多关于分离独立 state 变量的建议。
