...
一个路由匹配到的所有路由记录会暴露为 `$rout`e 对象 (还有在导航守卫中的路由对象) 的 `$route.matched` 数组。因此，我们需要遍历 `$route.matched` 来检查路由记录中的 `meta` 字段。
...
```
我们还可以利用路由元信息更细颗粒度地控制滚动。查看完整例子[请移步这里](https://github.com/vuejs/vue-router/blob/dev/examples/scroll-behavior/app.js)。
...
```
# 把组件按组分块
有时候我们想把某个路由下的所有组件都打包在同个异步块 (chunk) 中。只需要使用 命名 chunk，一个特殊的注释语法来提供 [chunk name](https://webpack.js.org/guides/code-splitting/) (需要 Webpack > 2.4)。
```js
