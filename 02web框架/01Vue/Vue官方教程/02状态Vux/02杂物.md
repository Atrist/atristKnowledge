...
# 测试 Mutation
Mutation 很容易被测试，因为它们仅仅是一些完全依赖参数的函数。这里有一个小技巧，如果你在 `store.j`s 文件中定义了 mutation，并且使用 ES2015 模块功能默认输出了 Vuex.Store 的实例，那么你仍然可以给 mutation 取个变量名然后把它输出去：
```js
const state = { ... }
...
```
如果在测试环境下有可用的 spy (比如通过 [Sinon.JS](https://sinonjs.org/))，你可以使用它们替换辅助函数 testAction：
