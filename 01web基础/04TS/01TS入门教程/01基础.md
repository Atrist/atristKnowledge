...
需要注意的是, 这里我们使用了简化的父类子类的关系来表达类型的兼容性, 而实际上TypeScript 在判断类型的兼容性时，比这种情况复杂很多，详细请参考[类型的兼容性（TODO)][]章节。
...
- [`export default ES6`](https://ts.xcatliu.com/basics/declaration-files.html#export-default) 默认导出
- [`export =`](https://ts.xcatliu.com/basics/declaration-files.html#export-1) commonjs 导出模块
- [`export as namespace UMD`](https://ts.xcatliu.com/basics/declaration-files.html#export-as-namespace) 库声明全局变量
...
export = jQuery;
```
其中用到了 `types` 和 `path` 两种不同的指令。它们的区别是：t`ypes` 用于声明对另一个库的依赖，而 `path` 用于声明对另一个文件的依赖。
