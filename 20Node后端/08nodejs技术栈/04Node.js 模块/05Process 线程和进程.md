...
![](../images/nodejs_process_test.png)

**进程间资源是独立的，关注的进程间通信问题。**

## 线程
线程是操作系统能够进行运算调度的**最小单位**，首先我们要清楚线程是隶属于**进程**的，被包含于进程之中。一个线程只能隶属于一个进程，但是一个进程是可以拥有多个线程的。
...
先看一段例子，运行下面程序，浏览器执行 http://127.0.0.1:3000/compute 大约每次需要 15657.310ms，也就意味下次用户请求需要等待 15657.310ms，[下文 Node.js 进程创建一节](https://www.nodejs.red/#/nodejs/process-threads?id=fork%e5%ad%90%e8%bf%9b%e7%a8%8b%e5%85%85%e5%88%86%e5%88%a9%e7%94%a8cpu%e8%b5%84%e6%ba%90) 将采用 `child_process.fork` 实现多个进程来处理。
...
在单核 CPU 系统之上我们采用 `单进程 + 单线程` 的模式来开发。在多核 CPU 系统之上，可以用过 `child_process.fork` 开启多个进程（Node.js 在 v0.8 版本之后新增了`Cluster` 来实现多进程架构） ，即 `多进程 + 单线程` 模式。注意：开启多进程不是为了解决高并发，主要是解决了单进程模式下 Node.js CPU 利用率不足的情况，充分利用多核 `CPU` 的性能。
...
以上仅列举了部分常用到功能点，除了 Process 之外 Node.js 还提供了 child_process 模块用来对子进程进行操作，在下文 [Nodejs进程创建一节](https://www.nodejs.red/#/nodejs/process-threads?id=nodejs%e8%bf%9b%e7%a8%8b%e5%88%9b%e5%bb%ba) 会讲述。
...
```
### fork子进程充分利用CPU资源
[上文单线程一节](https://www.nodejs.red/#/nodejs/process-threads?id=%e5%8d%95%e7%ba%bf%e7%a8%8b) 例子中，当 CPU 计算密度大的情况程序会造成阻塞导致后续请求需要等待，下面采用 child_process.fork 方法，在进行 `cpmpute` 计算时创建子进程，子进程计算完成通过 send 方法将结果发送给主进程，主进程通过 message 监听到信息后处理并退出。
...
```
>fork_compute.js
针对 [上文单线程一节](https://www.nodejs.red/#/nodejs/process-threads?id=%e5%8d%95%e7%ba%bf%e7%a8%8b) 的例子需要进行计算的部分拆分出来单独进行运算。
```js
const computation = () => {
    let sum = 0;
    console.info('计算开始');
...
![](../images/multi-process-architecture.png)
...
```
关于多进程端口占用问题，cnode 上有篇文章也可以看下 通[过源码解析 Node.js 中 cluster 模块的主要功能实现](https://cnodejs.org/topic/56e84480833b7c8a0492e20c)
...
参考守护进程一节 [守护进程编写](https://www.nodejs.red/#/nodejs/process-threads?id=%e5%ae%88%e6%8a%a4%e8%bf%9b%e7%a8%8b)
...
多进程模式下各个进程之间是相互独立的，例如用户登陆之后 session 的保存，如果保存在服务进程里，那么如果我有 4 个工作进程，每个进程都要保存一份这是没必要的，假设服务重启了数据也会丢失。多个 Web 服务也是一样的，还会出现我在 A 机器上创建了 Session，当负载均衡分发到 B 机器上之后还需要在创建一份。一般的做法是通过 `Redis` 或者 `数据库`来做数据共享。
